...
const randomKey = process.env.REACT_ERROR_OVERLAY_GLOBAL_HOOK || Math.random().toString(16).substr(2,6);

const ORIGIN_KEY = '__REACT_ERROR_OVERLAY_GLOBAL_HOOK__'

const PROPERTY_KEY = `${ORIGIN_KEY}$__${randomKey}`
...
function update() {
  ...
  loadingIframe.onload = function () {
    ...
    if (iframeDocument != null && iframeDocument.body != null) {
      ...
      script.innerHTML = iframeScript.replace(ORIGIN_KEY, PROPERTY_KEY);
...
console.log('[MED]', '您正在使用MED改造的hot reload框架！')

window[PROPERTY_KEY] =
  window[PROPERTY_KEY] || {};
window[PROPERTY_KEY].iframeReady = function iframeReady() {
  isIframeReady = true;
